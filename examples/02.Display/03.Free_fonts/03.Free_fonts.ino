...
void loop() {
  ...
  KBX.Lcd.print("Serif Bold 9pt");  // Print the font name onto the TFT screen

  KBX.Lcd.setFreeFont(FSB12);       // Select Free Serif 12 point font
  KBX.Lcd.println();                // Move cursor down a line
  KBX.Lcd.print("Serif Bold 12pt"); // Print the font name onto the TFT screen

  KBX.Lcd.setFreeFont(FSB18);       // Select Free Serif 12 point font
  KBX.Lcd.println();                // Move cursor down a line
  KBX.Lcd.print("Serif Bold 18pt"); // Print the font name onto the TFT screen

  KBX.Lcd.setFreeFont(FSB24);       // Select Free Serif 24 point font
  KBX.Lcd.println();                // Move cursor down a line
  KBX.Lcd.print("Serif Bold 24pt"); // Print the font name onto the TFT screen
  ...
  KBX.Lcd.drawString("Serif Bold 9pt", xpos, ypos, GFXFF);  // Draw the text string in the selected GFX free font
  ypos += KBX.Lcd.fontHeight(GFXFF);                      // Get the font height and move ypos down

  KBX.Lcd.setFreeFont(FSB12);
  KBX.Lcd.drawString("Serif Bold 12pt", xpos, ypos, GFXFF);
  ypos += KBX.Lcd.fontHeight(GFXFF);

  KBX.Lcd.setFreeFont(FSB18);
  KBX.Lcd.drawString("Serif Bold 18pt", xpos, ypos, GFXFF);
  ypos += KBX.Lcd.fontHeight(GFXFF);

  KBX.Lcd.setFreeFont(FSB24);
  KBX.Lcd.drawString("Serif Bold 24pt", xpos, ypos, GFXFF);
  ypos += KBX.Lcd.fontHeight(GFXFF);
  ...
  KBX.Lcd.drawString("Bold Italic 24pt", xpos, ypos, GFXFF);
  ypos += KBX.Lcd.fontHeight(GFXFF);
  ...
  KBX.Lcd.setTextColor(TFT_DARKGREY, TFT_BLACK);
  KBX.Lcd.setFreeFont(FSS12);
  KBX.Lcd.setTextDatum(TL_DATUM);
  KBX.Lcd.drawString("[Top left]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(TC_DATUM);
  KBX.Lcd.drawString("[Top centre]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(TR_DATUM);
  KBX.Lcd.drawString("[Top right]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(ML_DATUM);
  KBX.Lcd.drawString("[Middle left]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(MC_DATUM);
  KBX.Lcd.drawString("[Middle centre]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(MR_DATUM);
  KBX.Lcd.drawString("[Middle right]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(BL_DATUM);
  KBX.Lcd.drawString("[Bottom left]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(BC_DATUM);
  KBX.Lcd.drawString("[Bottom centre]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(BR_DATUM);
  KBX.Lcd.drawString("[Bottom right]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(L_BASELINE);
  KBX.Lcd.drawString("[Left baseline]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(C_BASELINE);
  KBX.Lcd.drawString("[Centre baseline]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);

  KBX.Lcd.fillRect(0, 80, 320, 80, TFT_BLACK);
  KBX.Lcd.setTextDatum(R_BASELINE);
  KBX.Lcd.drawString("[Right baseline]", 160, 120, GFXFF);
  drawDatumMarker(160,120);
  delay(1000);
...
void header(const char *string, uint16_t color)
{
  ...
  KBX.Lcd.setTextDatum(TC_DATUM);
  KBX.Lcd.drawString(string, 160, 2, 4); // Font 4 for fast drawing with background
}
