...
void loop() {
  ...
  KBX.Lcd.getTouchRaw(&x, &y);
  
//  Serial.printf("x: %i     ", x);

//  Serial.printf("y: %i     ", y);

//  Serial.printf("z: %i \n", KBX.Lcd.getTouchRawZ());
  KBX.Lcd.setCursor(0, 0);
  KBX.Lcd.printf("x: %i     ", x); KBX.Lcd.println();

  KBX.Lcd.printf("y: %i     ", y); KBX.Lcd.println();

  KBX.Lcd.printf("z: %i \n", KBX.Lcd.getTouchRawZ()); KBX.Lcd.println();
