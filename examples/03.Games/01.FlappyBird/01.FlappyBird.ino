...
// background
const unsigned int BCKGRDCOL = KBX.Lcd.color565(138,235,244);
// bird
const unsigned int BIRDCOL = KBX.Lcd.color565(255,254,174);
// pipe
const unsigned int PIPECOL  = KBX.Lcd.color565(99,255,78);
// pipe highlight
const unsigned int PIPEHIGHCOL  = KBX.Lcd.color565(250,255,250);
// pipe seam
const unsigned int PIPESEAMCOL  = KBX.Lcd.color565(0,0,0);
// floor
const unsigned int FLOORCOL = KBX.Lcd.color565(246,240,163);
// grass (col2 is the stripe color)
const unsigned int GRASSCOL  = KBX.Lcd.color565(141,225,87);
const unsigned int GRASSCOL2 = KBX.Lcd.color565(156,239,88);

// bird sprite
// bird sprite colors (Cx name for values to keep the array readable)
#define C0 BCKGRDCOL
#define C1 KBX.Lcd.color565(195,165,75)
#define C2 BIRDCOL
#define C3 TFT_WHITE
#define C4 TFT_RED
#define C5 KBX.Lcd.color565(251,216,114)
...
#define drawPixel(a, b, c) KBX.Lcd.setAddrWindow(a, b, a, b); KBX.Lcd.pushColor(c)
...
void game_loop() {
  ...
  // black line
  KBX.Lcd.drawFastHLine(0, GAMEH, TFTW, TFT_BLACK);
  // grass and stripe
  KBX.Lcd.fillRect(0, GAMEH+1, TFTW2, GRASSH, GRASSCOL);
  KBX.Lcd.fillRect(TFTW2, GAMEH+1, TFTW2, GRASSH, GRASSCOL2);
  // black line
  KBX.Lcd.drawFastHLine(0, GAMEH+GRASSH, TFTW, TFT_BLACK);
  // mud
  KBX.Lcd.fillRect(0, GAMEH+GRASSH+1, TFTW, FLOORH-GRASSH, FLOORCOL);
  ...
  while (1) {
    ...
    if (pipes.x >= 0 && pipes.x < TFTW) {
      // pipe color
      KBX.Lcd.drawFastVLine(pipes.x+3, 0, pipes.gap_y, PIPECOL);
      KBX.Lcd.drawFastVLine(pipes.x+3, pipes.gap_y+GAPHEIGHT+1, GAMEH-(pipes.gap_y+GAPHEIGHT+1), PIPECOL);
      // highlight
      KBX.Lcd.drawFastVLine(pipes.x, 0, pipes.gap_y, PIPEHIGHCOL);
      KBX.Lcd.drawFastVLine(pipes.x, pipes.gap_y+GAPHEIGHT+1, GAMEH-(pipes.gap_y+GAPHEIGHT+1), PIPEHIGHCOL);
    ...
    // erase behind pipe
    if (pipes.x <= TFTW) KBX.Lcd.drawFastVLine(pipes.x+PIPEW, 0, GAMEH, BCKGRDCOL);
    ...
    KBX.Lcd.drawFastVLine( grassx    %TFTW, GAMEH+1, GRASSH-1, GRASSCOL);
    KBX.Lcd.drawFastVLine((grassx+64)%TFTW, GAMEH+1, GRASSH-1, GRASSCOL2);
    ...
    // if bird has passed the pipe increase score
      ...
      KBX.Lcd.setTextColor(BCKGRDCOL);
      KBX.Lcd.setCursor( TFTW2, 4);
      KBX.Lcd.print(score);
      // set text color back to white for new score
      KBX.Lcd.setTextColor(TFT_WHITE);
...
void game_start() {
  ...
  KBX.Lcd.fillRect(10, TFTH2 - 20, TFTW-20, 1, TFT_WHITE);
  KBX.Lcd.fillRect(10, TFTH2 + 32, TFTW-20, 1, TFT_WHITE);
  KBX.Lcd.setTextColor(TFT_WHITE);
  KBX.Lcd.setTextSize(3);
  // half width - num char * char width in pixels
  KBX.Lcd.setCursor( TFTW2-(6*9), TFTH2 - 16);
  KBX.Lcd.println("FLAPPY");
  KBX.Lcd.setTextSize(3);
  KBX.Lcd.setCursor( TFTW2-(6*9), TFTH2 + 8);
  KBX.Lcd.println("-BIRD-");
  KBX.Lcd.setTextSize(2);
  KBX.Lcd.setCursor( 10, TFTH2 - 36);
  KBX.Lcd.println("M5Stack");
  KBX.Lcd.setCursor( TFTW2 - (17*9), TFTH2 + 36);
  KBX.Lcd.println("Premi il bottone centrale");
  while (1) {
...
void game_over() {
  ...
  if(score>maxScore)
  {
    ...
    KBX.Lcd.setTextSize(2); 
    KBX.Lcd.setCursor( TFTW2 - (13*6), TFTH2 - 26);
    KBX.Lcd.println("NEW HIGHSCORE");
  }
  
  KBX.Lcd.setTextColor(TFT_WHITE);
  KBX.Lcd.setTextSize(3);
  // half width - num char * char width in pixels
  KBX.Lcd.setCursor( TFTW2 - (9*9), TFTH2 - 6);
  KBX.Lcd.println("GAME OVER");
  KBX.Lcd.setTextSize(2);
  KBX.Lcd.setCursor( 10, 10);
  KBX.Lcd.print("score: ");
  KBX.Lcd.print(score);
  KBX.Lcd.setCursor( TFTW2 - (12*6), TFTH2 + 18);
  KBX.Lcd.println("press button");
  KBX.Lcd.setCursor( 10, 28);
  KBX.Lcd.print("Max Score:");
  KBX.Lcd.print(maxScore);
  while (1) {
    // wait for push button
      if(KBX.BtnB.wasPressed()) {
        break;
