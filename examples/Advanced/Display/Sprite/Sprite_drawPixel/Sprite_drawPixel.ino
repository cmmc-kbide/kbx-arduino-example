...
TFT_eSprite spr = TFT_eSprite(&KBX.Lcd);
...
void loop(void)
{
  ...
  spr.pushSprite(KBX.Lcd.width() / 2 - WIDTH / 2, KBX.Lcd.height() / 2 - HEIGHT / 2);
  spr.pushSprite(KBX.Lcd.width() - WIDTH + 40, KBX.Lcd.height() - HEIGHT + 40);
  ...
  int x = KBX.Lcd.width() / 2  -  WIDTH / 2;
  int y = KBX.Lcd.height() / 2 - HEIGHT / 2;
  ...
  while (true)
  {
    ...
    if (x < -WIDTH/2) dx = 1;
    if (x >= KBX.Lcd.width()-WIDTH/2) dx = -1;
    if (y < -HEIGHT/2) dy = 1;
    if (y >= KBX.Lcd.height()-HEIGHT/2) dy = -1;
