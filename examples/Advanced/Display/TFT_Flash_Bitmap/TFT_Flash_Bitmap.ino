...
void loop()
{
  ...
  drawIcon(info,  random(KBX.Lcd.width() -  infoWidth), random(KBX.Lcd.height() -  infoHeight),  infoWidth,  infoHeight);
  drawIcon(alert, random(KBX.Lcd.width() - alertWidth), random(KBX.Lcd.height() - alertHeight), alertWidth, alertHeight);
  drawIcon(close, random(KBX.Lcd.width() - closeWidth), random(KBX.Lcd.height() - closeHeight), alertWidth, closeHeight);

  // Clear screen after 100 x 3 = 300 icons drawn
  if (100 == count++) {
    count = 1;
    KBX.Lcd.setRotation(2 * random(2)); // Rotate randomly to clear display left>right or right>left to reduce monotony!
    KBX.Lcd.fillScreen(TFT_BLACK);
...
void drawIcon(const unsigned short* icon, int16_t x, int16_t y, int8_t width, int8_t height) {
  ...
  KBX.Lcd.setWindow(x, y, x + width - 1, y + height - 1);
  ...
  for (int i = 0; i < nb; i++) {
    ...
    KBX.Lcd.pushColors(pix_buffer, BUFF_SIZE);
  }
  ...
  if (np) {
    ...
    KBX.Lcd.pushColors(pix_buffer, np);
  }
}
