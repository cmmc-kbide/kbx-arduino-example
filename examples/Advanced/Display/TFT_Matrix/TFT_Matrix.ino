...
void loop(void) {
  ...
  for (int j = 0; j < 600; j += TEXT_HEIGHT) {
    for (int i = 0; i < 40; i++) {
      ...
      KBX.Lcd.setTextColor(pos[i] << 5, ILI9341_BLACK); // Set the green character brightness
      if (pos[i] == 63) KBX.Lcd.setTextColor(ILI9341_WHITE, ILI9341_BLACK); // Draw white character
      xPos += KBX.Lcd.drawChar(random(32, 128), xPos, yDraw, 1); // Draw the character
    }
  ...
  //KBX.Lcd.setRotation(2);
  //KBX.Lcd.setTextColor(63 << 5, ILI9341_BLACK);
  //KBX.Lcd.drawCentreString("MATRIX",120,60,4);
...
void setupScrollArea(uint16_t TFA, uint16_t BFA) {
  ...
  KBX.Lcd.writedata(TFA >> 8);
  KBX.Lcd.writedata(TFA);
  KBX.Lcd.writedata((320 - TFA - BFA) >> 8);
  KBX.Lcd.writedata(320 - TFA - BFA);
  KBX.Lcd.writedata(BFA >> 8);
...
void scrollAddress(uint16_t VSP) {
  KBX.Lcd.writecommand(ILI9341_VSCRSADD); // Vertical scrolling start address
