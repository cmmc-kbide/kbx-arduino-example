...
void initgame() {
  ...
  // KBX.Lcd.fillRect(0,h-26,w,239,GREY);

  // KBX.Lcd.setTextDatum(TC_DATUM);
  // KBX.Lcd.setTextColor(WHITE,GREY);
  // KBX.Lcd.drawString("TFT_eSPI example", w/2, h-26 , 4);
}
...
void midline() {
  ...
  KBX.Lcd.setWindow(dashline_x,0,dashline_x+dashline_w-1,h);
  
  for(int16_t i = 0; i < dashline_n; i+=2) {
    KBX.Lcd.pushColor(WHITE, dashline_w*dashline_h); // push dash pixels
    KBX.Lcd.pushColor(BLACK, dashline_w*dashline_h); // push gap pixels
  }
}
...
void lpaddle() {
  ...
  if (lpaddle_d == 1) {
    KBX.Lcd.fillRect(lpaddle_x, lpaddle_y, paddle_w, 1, BLACK);
  } 
  else if (lpaddle_d == -1) {
    KBX.Lcd.fillRect(lpaddle_x, lpaddle_y + paddle_h - 1, paddle_w, 1, BLACK);
  }
  ...
  KBX.Lcd.fillRect(lpaddle_x, lpaddle_y, paddle_w, paddle_h, WHITE);
}

void rpaddle() {
  
  if (rpaddle_d == 1) {
    KBX.Lcd.fillRect(rpaddle_x, rpaddle_y, paddle_w, 1, BLACK);
  } 
  else if (rpaddle_d == -1) {
    KBX.Lcd.fillRect(rpaddle_x, rpaddle_y + paddle_h - 1, paddle_w, 1, BLACK);
  }
  ...
  KBX.Lcd.fillRect(rpaddle_x, rpaddle_y, paddle_w, paddle_h, WHITE);
}
...
void ball() {
  ...
  //KBX.Lcd.fillRect(oldball_x, oldball_y, ball_w, ball_h, BLACK);
  KBX.Lcd.drawRect(oldball_x, oldball_y, ball_w, ball_h, BLACK); // Less TFT refresh aliasing than line above for large balls
  KBX.Lcd.fillRect(   ball_x,    ball_y, ball_w, ball_h, WHITE);
  oldball_x = ball_x;
  oldball_y = ball_y;
}
