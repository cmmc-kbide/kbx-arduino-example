...
void setup() {
  ...
  //KBX.Lcd.fastSetup(); // Prepare plot window range for fast pixel plotting
}
...
void loop()
{
  ...
  for(int i = 0; i < NSTARS; ++i)
  {
    ...
    else
    {
      ...
      KBX.Lcd.drawPixel(old_screen_x, old_screen_y,TFT_BLACK);
      ...
      if (sz[i] > 1)
      {
        ...
        if (screen_x >= 0 && screen_y >= 0 && screen_x < 320 && screen_y < 240)
        {
          uint8_t r, g, b;
          r = g = b = 255 - sz[i];
          KBX.Lcd.drawPixel(screen_x, screen_y, KBX.Lcd.color565(r,g,b));
        }
        else
