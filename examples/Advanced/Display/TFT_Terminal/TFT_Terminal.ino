...
void setup() {
  ...
  KBX.begin();
  // KBX.Lcd.setRotation(5); // Must be setRotation(0) for this sketch to work correctly
  ...
  KBX.Lcd.fillRect(0,0,320,TEXT_HEIGHT, TFT_BLUE);
  KBX.Lcd.drawCentreString(" Serial Terminal - 115200 baud ",320/2,0,2);
...
void loop(void) {
  ...
  //  if (change_colour){
  //  change_colour = 0;
  //  if (selected == 1) {KBX.Lcd.setTextColor(TFT_CYAN, TFT_BLACK); selected = 0;}
  //  else {KBX.Lcd.setTextColor(TFT_MAGENTA, TFT_BLACK); selected = 1;}
  //}
  ...
  while (Serial.available()) {
    ...
    if (data > 31 && data < 128) {
      xPos += KBX.Lcd.drawChar(data,xPos,yDraw,2);
...
int scroll_line() {
  ...
  // KBX.Lcd.fillRect(0,yStart,blank[(yStart-TOP_FIXED_AREA)/TEXT_HEIGHT],TEXT_HEIGHT, TFT_BLACK);
  KBX.Lcd.fillRect(0,yStart,320,TEXT_HEIGHT, TFT_BLACK);
...
void setupScrollArea(uint16_t tfa, uint16_t bfa) {
  KBX.Lcd.writecommand(ILI9341_VSCRDEF); // Vertical scroll definition
  KBX.Lcd.writedata(tfa >> 8);           // Top Fixed Area line count
  KBX.Lcd.writedata(tfa);
  KBX.Lcd.writedata((YMAX-tfa-bfa)>>8);  // Vertical Scrolling Area line count
  KBX.Lcd.writedata(YMAX-tfa-bfa);
  KBX.Lcd.writedata(bfa >> 8);           // Bottom Fixed Area line count
  KBX.Lcd.writedata(bfa);
}
...
void scrollAddress(uint16_t vsp) {
  KBX.Lcd.writecommand(ILI9341_VSCRSADD); // Vertical scrolling pointer
  KBX.Lcd.writedata(vsp>>8);
  KBX.Lcd.writedata(vsp);
}
